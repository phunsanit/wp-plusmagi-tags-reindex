...
test.describe('PlusMagi Tags Reindex — Block Editor', () => {
    ...
    test('Custom Tags Panel renders and auto-splits comma-separated tags', async ({ page }) => {
        ...
        await page.waitForTimeout(100); // รอให้ React อัปเดต State จากการ Fill ให้เสร็จสมบูรณ์ก่อน
        ...
        // 4. ทดสอบเพิ่ม Tag หลายคำพร้อมกันด้วยข้อความที่มีคอมม่าคั่น (จำลองการ Paste)
        await tagInput.fill(`PlaywrightTagD_${uniqueId}, PlaywrightTagE_${uniqueId}`);
        await expect(tagInput).toHaveValue('');

        // คาดหวังว่าจะมีชื่อ Tag ทั้งหมด ปรากฏขึ้นมาแสดงใน Panel ทันที
