...
resolve_slug() {
	...
	# เปลี่ยนวิธีอ่าน package.json ให้ปลอดภัยขึ้น ไม่พังถ้าไม่มีไฟล์
...
main() {
	...
	# ปรับปรุงการหา Version ให้ยืดหยุ่นขึ้น ไม่พังถ้ารูปแบบคอมเมนต์ต่างกันนิดหน่อย
	...
	(
		...
		chmod a-rwx "$zip_filename"

		# ปรับการทำงานของ stat ให้ปลอดภัยบน macOS M5
		local user_group
		user_group=$(stat -f "%u:%g" "$SOURCE_DIR" 2>/dev/null || stat --format="%U:%G" "$SOURCE_DIR" 2>/dev/null || echo "")
		if [[ -n "$user_group" ]]; then
			chown "$user_group" "$zip_filename"
		fi
	)
